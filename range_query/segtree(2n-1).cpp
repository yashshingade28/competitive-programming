   ...
   public:
    ...
    node get(int idx, int l, int r, int ll, int rr) {
        ...
        if (r <= mid) {
            ...
            return combine(get(idx + 1, l, mid, ll, mid), get(ridx, mid + 1, r, mid + 1, rr));  // query type
        }
    }

    node get(int l, int r) { return get(0, l, r, 0, n - 1); }

    node get(int p) { return get(0, p, p, 0, n - 1); }
    ...
    void update(const int& p, const int& to) { update(0, p, to, 0, n - 1); }
};
